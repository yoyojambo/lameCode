...
const deleteChallengeTest = `-- name: DeleteChallengeTest :exec
...
const getChallengeWithTests = `-- name: GetChallengeWithTests :one
SELECT 
    c.id,
    c.title,
    c.description,
...
func (q *Queries) GetChallengeWithTests(ctx context.Context, id int64) (Challenge, error) {
	row := q.db.QueryRowContext(ctx, getChallengeWithTests, id)
	var i Challenge
	err := row.Scan(
		&i.ID,
		&i.Title,
...
const getProblemsForAdmin = `-- name: GetProblemsForAdmin :many
SELECT 
    c.id,
    c.title,
    c.description,
...
type GetProblemsForAdminRow struct {
	ID              int64  `json:"id"`
	...
	SolverCount     int64  `json:"solver_count"`
	SubmissionCount int64  `json:"submission_count"`
}

func (q *Queries) GetProblemsForAdmin(ctx context.Context) ([]GetProblemsForAdminRow, error) {
	rows, err := q.db.QueryContext(ctx, getProblemsForAdmin)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetProblemsForAdminRow
	for rows.Next() {
		var i GetProblemsForAdminRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Description,
...
WHERE id = ?
RETURNING id, title, description, difficulty, test_count, created_at, updated_at
`

func (q *Queries) UpdateChallenge(ctx context.Context, title string, description string, difficulty int64, iD int64) (Challenge, error) {
...
const updateChallengeTest = `-- name: UpdateChallengeTest :one
UPDATE challenge_tests 
SET input_data = ?, expected_output = ?
WHERE id = ?
RETURNING id, challenge_id, input_data, expected_output, created_at
`

func (q *Queries) UpdateChallengeTest(ctx context.Context, inputData string, expectedOutput string, iD int64) (ChallengeTest, error) {
	row := q.db.QueryRowContext(ctx, updateChallengeTest, inputData, expectedOutput, iD)
	var i ChallengeTest
	err := row.Scan(
		&i.ID,
		&i.ChallengeID,
